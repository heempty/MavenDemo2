    ...
    switch (msg.type) {
        ...
        case 'custom':
            ...
            if (content.type === 1) {
                ...
                str = "<div class='f-maxWid'>" + str + "</div>";*/
                var re = /(http:\/\/[\w.\/]+)(?![^<]+>)/gi; // 识别链接
                var strcont = msg.content;
                strcont.replace("\\","");  // 转换斜杠 '\'
                strcont = JSON.parse(strcont); // 转成对象形式
                var title = _$escape(strcont.title);
                var content = _$escape(strcont.content);
                // console.log("title = "+title+" content = "+content)
                content = content.replace(re, "<a href='$1' target='_blank'>$1</a>");

                title = buildEmoji(title);
                content = buildEmoji(content);

                var div_cont = "<div class='div_cont_title'>"+title+"</div>";
                div_cont += "<div class='div_cont_content'>"+content+"</div>";

                if (strcont.url){
                    var url = "http://img.800pharm.com/images//pms/upload/images/original/072017/10135/abfba67e24544b8e7d0e8e3cf481c4a4162807e5.jpg";
                    div_cont +='<a href="' + url + '?imageView" target="_blank"><img onload="loadImg()" data-src="' + url + '" src="' + url + '?imageView&thumbnail=200x0&quality=85"/></a>';
                }

                str = "<div class='f-maxWid'>" + div_cont + "</div>"
                break;
            }
            break;
        case 'robot':
