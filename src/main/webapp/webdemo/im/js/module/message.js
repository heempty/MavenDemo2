...
YX.fn.message = function() {
  ...
  this.$sendBtn2.on('click', this.sendCustomMessageToUser.bind(this));
...
YX.fn.sendTextMessage = function() {
  ...
  if (!!to && !!text) {
    if (text.length > 500) {
      ...
      // 获取好友的消息订阅，离线就发送消息
      var status = this.cache.getMultiPortStatus("571283626");
      console.log("user status --> ? |"+status+"|");
        if (status == "Web在线"||status == ""){
            var content = {title:"testHello",content:"test,这是测试消息"};
            this.mysdk.sendCustomMessage('p2p',"1115348101",content,this.sendMsgDone.bind(this));
        }
    }
  }
};
// 发送自定义消息
YX.fn.sendCustomMessageToUser = function () {
    var scene = this.crtSessionType,
        to = this.crtSessionAccount,
        contentval = this.$messageText2.val().trim(),
        testtitle = this.$testtitle.val().trim()
        ;
        var content = {title:testtitle,content:contentval};
    this.mysdk.sendCustomMessage('p2p', to, content, null)
};
YX.fn.sendCustomMessageToUser2 = function (to,title,contents,url) {
    var content = {title:title,content:contents,url:url};
    this.mysdk.sendCustomMessage('p2p', to, content, this.sendMsgDone.bind(this))
}
/**
 * 发送消息完毕后的回调
 * @param error：消息发送失败的原因 对应文档 Class: NIMError
 * @param msg：消息主体，类型分为文本、文件、图片、地理位置、语音、视频、自定义消息，通知等  对应文档Class: IMMessage
 */
YX.fn.sendMsgDone = function(error, msg) {
  console.log("--------发送消息完毕后的回调---------",msg)
